...
namespace Projectfinal
{
    public partial class Form11 : Form
    {
        ...
        public Form11(string court, DateTime date, int userId,string userEmail, string userfname, string userlname)
        {
            InitializeComponent();
            LoadTimeSlots();
            selectedCourt = court;
        ...
        private List<(TimeSpan start, TimeSpan end)> GetBookedTimeSlots()
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT StartTime, EndTime FROM bookings WHERE BookingDate = @date AND CourtID = 1";
        ...
        private void comboBoxStartTime_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            // ดึงเวลาเริ่มต้นที่ผู้ใช้เลือกจาก ComboBox
            startTimeSelected = TimeSpan.Parse(comboBoxStartTime.SelectedItem.ToString());
        ...
        private void comboBoxEndTime_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            // ดึงเวลาสิ้นสุดที่ผู้ใช้เลือกจาก ComboBox
            endTimeSelected = TimeSpan.Parse(comboBoxEndTime.SelectedItem.ToString());
        }
