...
namespace Projectfinal
{
    public partial class Form14 : Form
    {
        ...
        public Form14(string court, DateTime date, int userId, string userEmail, string userfname, string userlname)
        {
            InitializeComponent();
            selectedCourt = court;
            selectedDate = date;
        ...
        private List<(TimeSpan start, TimeSpan end)> GetBookedTimeSlots()
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT StartTime, EndTime FROM bookings WHERE BookingDate = @date AND CourtID = 4";
